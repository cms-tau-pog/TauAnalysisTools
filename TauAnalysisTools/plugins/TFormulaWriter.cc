#include "TauAnalysisTools/TauAnalysisTools/plugins/TFormulaWriter.h"
...
void TFormulaWriter::analyze(const edm::Event&, const edm::EventSetup&)
{
  ...
  for ( std::vector<jobEntryType*>::iterator job = jobs_.begin(); job != jobs_.end(); ++job )
  {
    ...
    delete inputFile;
    if ( !formula ) throw cms::Exception("TFormulaWriter") << " Failed to load TFormula = " << (*job)->formulaName_.data() << " from file = " << (*job)->inputFileName_ << " !!\n";

    edm::Service<cond::service::PoolDBOutputService> dbService;
    if ( !dbService.isAvailable() ) throw cms::Exception("TFormulaWriter") << " Failed to access PoolDBOutputService !!\n";
