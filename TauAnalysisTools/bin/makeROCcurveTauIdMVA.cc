...
namespace
{
  struct plotEntryType
  {
    ...
    ~plotEntryType() { delete histogram_; }
    ...
    {
      if ( (minTauPt_ < 0. || tauPt > minTauPt_) && (maxTauPt_ < 0. || tauPt < maxTauPt_) )
  ...
  void fillHistograms(TTree* tree, const std::string& discriminator,
		      ...
		      unsigned reportEvery)
  {
    ...
    TTreeFormula* treeFormula = new TTreeFormula(treeFormulaName.data(), discriminator.data(), tree);
    std::cout << " treeFormula = " << discriminator.data() << std::endl;
    ...
    int mode = -1;
    Float_t logTauPt, tauPt, discrim_val;
    if ( branchNameLogTauPt != "" && branchNameTauPt == "" )
    {
      std::cout << "mode: logTauPt" << std::endl;
      tree->SetBranchAddress(branchNameLogTauPt.data(), &logTauPt);
      mode = kLogTauPt;
    }
    else if ( branchNameLogTauPt == "" && branchNameTauPt != "" )
    {
      std::cout << "mode: tauPt" << std::endl;
      tree->SetBranchAddress(branchNameTauPt.data(), &tauPt);
      mode = kTauPt;
    }

    //tree->SetBranchAddress(discriminator.data(), &discrim_val);//'TMath_Log_TMath_Max_1.,recTauPt__'

    Float_t evtWeight = 1.0;
    if ( branchNameEvtWeight != "" ) tree->SetBranchAddress(branchNameEvtWeight.data(), &evtWeight);
    ...
    {
      if ( iEntry > 0 && (iEntry % reportEvery) == 0 ) std::cout << "processing Entry " << iEntry << std::endl;
      ...
      if ( iEntry > 0 && (iEntry % reportEvery) == 0 )
        std::cout << "\tdiscriminator " << discriminator << "; tauPt_value: " << tauPt_value << std::endl; // << "; discrim_val: " << discrim_val

      for ( std::vector<plotEntryType*>::iterator plotEntry = plotEntries.begin(); plotEntry != plotEntries.end(); ++plotEntry )
...
int main(int argc, char* argv[])
{
  ...
  if ( cfgMakeROCcurveTauIdMVA.exists("preselection_signal") ) preselection_signal = cfgMakeROCcurveTauIdMVA.getParameter<std::string>("preselection_signal");
  else preselection_signal = cfgMakeROCcurveTauIdMVA.getParameter<std::string>("preselection");
  if ( preselection_signal.length() >= 1 ) preselection_signal = Form("(%s)", preselection_signal.data());

  std::string preselection_background;
  if ( cfgMakeROCcurveTauIdMVA.exists("preselection_background") ) preselection_background = cfgMakeROCcurveTauIdMVA.getParameter<std::string>("preselection_background");
  else preselection_background = cfgMakeROCcurveTauIdMVA.getParameter<std::string>("preselection");
  if ( preselection_background.length() >= 1 ) preselection_background = Form("(%s)", preselection_background.data());
  ...
  if ( cfgMakeROCcurveTauIdMVA.exists("classId_signal") && cfgMakeROCcurveTauIdMVA.exists("classId_background") )
  {
    ...
    std::cout << "preselection_signal: " << preselection_signal << std::endl;
    ...
    std::cout << "preselection_background: " << preselection_background << std::endl;

    ++numMethods_signal_and_background_separation;
  }
  if ( numMethods_signal_and_background_separation != 1 )
    throw cms::Exception("makeROCcurveTauIdMVA") << "Need to specify either Configuration parameters 'signalSamples' and 'backgroundSamples' or 'classId_signal' plus 'classId_background' !!\n";
  ...
  if ( (branchNameLogTauPt == "" && branchNameTauPt == "") || (branchNameLogTauPt != "" && branchNameTauPt != "") )
    throw cms::Exception("makeROCcurveTauIdMVA") << "Need to set either Configuration parameters 'branchNameLogTauPt' or 'branchNameTauPt' to non-zero values !!\n";
  ...
  for ( vstring::const_iterator inputFileName = inputFiles.files().begin(); inputFileName != inputFiles.files().end(); ++inputFileName )
  {
    ...
    {
      std::cout << "\tAddFile inputFile " << inputFileName->data() << std::endl;
      tree_signal->AddFile(inputFileName->data());
  ...
  for (vstring::const_iterator branchToKeep_expressions = branchesToKeep_expressions.begin(); branchToKeep_expressions != branchesToKeep_expressions.end(); ++branchToKeep_expressions)
    std::cout << " branchToKeep_expressions = " << *branchToKeep_expressions << std::endl;
  ...
  // PRESELECTING SG
    ...
    for ( std::vector<pdouble>::const_iterator tauPtBin = tauPtBins.begin(); tauPtBin != tauPtBins.end(); ++tauPtBin )
    {
  ...
  // PRESELECTING BG
    ...
    for ( std::vector<TTree*>::iterator it = treesToDelete.begin(); it != treesToDelete.end(); ++it )
      delete (*it);
  ...
  for ( size_t iTauPtBin = 0; iTauPtBin < numTauPtBins; ++iTauPtBin )
  {
    ...
    TH1* histogram_signal = plotEntries_signal[iTauPtBin]->histogram_;// const
    histogram_signal->Draw();
    TH1* histogram_background = plotEntries_background[iTauPtBin]->histogram_;// const
    histogram_background->SetFillColor(kRed);
    histogram_background->SetLineColor(kRed);
    histogram_background->Draw("same");
    //makeROCcurve(const std::string& graphName, const TH1* histogram_signal, const TH1* histogram_background)
    ...
    img->FromPad(c1);
    img->WriteImage(Form("/afs/desy.de/user/g/glusheno/RWTH/MVAtraining/CMSSW_9_4_2/src/TauAnalysisTools/TauAnalysisTools/test/canvas%i.png", int(iTauPtBin)));
    delete c1;
    delete img;
  ...
  outputFile->cd();
  for ( std::vector<plotEntryType*>::iterator plotEntry = plotEntries_signal.begin(); plotEntry != plotEntries_signal.end(); ++plotEntry )
  {
    std::cout << "writing histogram = " << (*plotEntry)->histogram_->GetName() << std::endl;
    (*plotEntry)->histogram_->Write();
  }
  for ( std::vector<plotEntryType*>::iterator plotEntry = plotEntries_background.begin(); plotEntry != plotEntries_background.end(); ++plotEntry )
  {
    std::cout << "writing histogram = " << (*plotEntry)->histogram_->GetName() << std::endl;
    (*plotEntry)->histogram_->Write();
  }
  for ( std::vector<TGraph*>::iterator graphROCcurve = graphsROCcurve.begin(); graphROCcurve != graphsROCcurve.end(); ++graphROCcurve )
  {
  ...
  for ( std::vector<plotEntryType*>::iterator it = plotEntries_signal.begin(); it != plotEntries_signal.end(); ++it )
  {
    std::cout << "deleting histogram = " << (*it)->histogram_->GetName() << std::endl;
    delete (*it);
  }
  for ( std::vector<plotEntryType*>::iterator it = plotEntries_background.begin(); it != plotEntries_background.end(); ++it )
  {
    std::cout << "deleting histogram = " << (*it)->histogram_->GetName() << std::endl;
    delete (*it);
  }
  for ( std::vector<TGraph*>::iterator it = graphsROCcurve.begin(); it != graphsROCcurve.end(); ++it )
  {
