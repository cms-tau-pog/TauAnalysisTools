...
int main(int argc, char* argv[])
{
	...
	for ( vParameterSet::const_iterator cfgGraph = cfgGraphs.begin(); cfgGraph != cfgGraphs.end(); ++cfgGraph )
	{
	...
	for ( std::vector<pdouble>::const_iterator tauPtBin = tauPtBins.begin(); tauPtBin != tauPtBins.end(); ++tauPtBin )
	{
		for ( vstring::const_iterator inputFileName = inputFiles.files().begin(); inputFileName != inputFiles.files().end(); ++inputFileName )
		{
			...
			if ( !inputFile )
			 throw cms::Exception("showROCcurvesTauIdMVA") << "Failed to open input file = " << (*inputFileName) << " !!\n";

			for ( std::vector<graphEntryType*>::iterator graphEntry = graphEntries.begin(); graphEntry != graphEntries.end(); ++graphEntry )
			{
				...
				if ( graph )
				{
					if ( (*graphEntry)->graph_ )
						throw cms::Exception("showROCcurvesTauIdMVA") << "Graphs of name = " << graphName_full << " found in multiple input files !!\n";
		...
		for ( std::vector<graphEntryType*>::iterator graphEntry = graphEntries.begin(); graphEntry != graphEntries.end(); ++graphEntry )
		{
			if ( (*graphEntry)->graph_ )
			{
				...
				if ( (*graphEntry)->graph_->GetN() < 10 )
				{ // CV: remove "trivial" points (signal and background events either all pass or all fail)
				...
				throw cms::Exception("showROCcurvesTauIdMVA") << "Graph of name = " << (*graphEntry)->graphName_ << " not found in any input file !!\n";
		}

		int numLegendEntries = 0;
		for ( std::vector<graphEntryType*>::const_iterator graphEntry = graphEntries.begin(); graphEntry != graphEntries.end(); ++graphEntry )
			if ( (*graphEntry)->graph_ && (*graphEntry)->legendEntry_ != "" )
				++numLegendEntries;
	...
	for ( std::vector<graphEntryType*>::iterator it = graphEntries.begin(); it != graphEntries.end(); ++it )
		delete (*it);
