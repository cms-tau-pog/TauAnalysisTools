...
{
	if ((!(histogram_numerator->GetNbinsX() == histogram_denominator->GetNbinsX()))
		|| (!(histogram_numerator->GetXaxis()->GetXmin() == histogram_denominator->GetXaxis()->GetXmin()))
		|| (!(histogram_numerator->GetXaxis()->GetXmax() == histogram_denominator->GetXaxis()->GetXmax())))
	{
	...
	TGraphAsymmErrors* graph = new TGraphAsymmErrors(nBins, x.GetArray(), y.GetArray(), dxDown.GetArray(), dxUp.GetArray(), dyDown.GetArray(), dyUp.GetArray());
...
struct plotEntryType
{
	plotEntryType(const std::string& name, double mvaCut)
		: name_(name),
			mvaCut_(mvaCut),
			histogramPt_numerator_(0), histogramPt_denominator_(0),
			histogramEta_numerator_(0), histogramEta_denominator_(0),
			histogramNvtx_numerator_(0), histogramNvtx_denominator_(0),
	...
	void bookHistograms()
	{
		...
		double ptBinning[ptNumBins + 1] = { 20., 22.5, 25., 27.5, 30., 32.5, 35., 37.5, 40., 45., 50., 55., 60., 70., 80., 90., 100., 125., 150., 175., 200., 250., 300., 400., 500., 600., 700., 800., 1000., 1200., 1500., 5000. };
				 ...
				 const std::string& inputFileDirs = "")
{
	...
	{
		std::cout << treeName.data() << " tree; Problem: " << "tauPtMode: " << tauPtMode << std::endl;
		tree->SetBranchAddress("TMath_Log_TMath_Max_1.,recTauPt__", &recLogTauPt);}
	else
	{
		std::cout << treeName << " tree; Problem: " << "tauPtMode: " << tauPtMode << std::endl;
		assert(0);
	}
	...
	{
		std::cout << treeName << " tree; Problem: " << "tauEtaMode: " << tauEtaMode << std::endl;
		assert(0);
	}
	...
	for (int iEntry = 0; iEntry < numEntries && (iEntry < maxEvents || maxEvents == -1); ++iEntry)
	{
		...
		if (tauPtMode == kLogTauPt) recTauPt = TMath::Exp(recLogTauPt);
		if (tauEtaMode == kTauEta) recTauAbsEta = TMath::Abs(recTauEta);
		...
		if (classId == classId_signal)
			plots_signal->fillHistograms(mvaOutput_normalized, recTauPt, recTauAbsEta, numVertices, evtWeight);
		else if (classId == classId_background)
			plots_background->fillHistograms(mvaOutput_normalized, recTauPt, recTauAbsEta, numVertices, evtWeight);
	}
...
void plotTauIdMVAEfficiency_and_FakeRate()
{
	...
	*/
	...
	for (std::vector<mvaEntryType*>::iterator it = mvaEntries.begin();	it != mvaEntries.end(); ++it)
		delete (*it);
}
