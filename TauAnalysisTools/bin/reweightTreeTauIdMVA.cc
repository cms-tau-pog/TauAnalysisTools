...
namespace
{
  ...
  {
    std::cout << "fillHistogramsForPtVsEtaReweighting<>: begin" << std::endl;
    Float_t pt;
    ...
    std::cout << "fillHistogramsForPtVsEtaReweighting<>: end" << std::endl;
  }
...
int main(int argc, char* argv[])
{
  ...
  if ( !edm::readPSetsFrom(argv[1])->existsAs<edm::ParameterSet>("process") )
    throw cms::Exception("reweightTreeTauIdMVA") << "No ParameterSet 'process' found in configuration file = " << argv[1] << " !!\n";
