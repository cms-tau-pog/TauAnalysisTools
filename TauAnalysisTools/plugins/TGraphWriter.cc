#include "TauAnalysisTools/TauAnalysisTools/plugins/TGraphWriter.h"
...
void TGraphWriter::analyze(const edm::Event&, const edm::EventSetup&)
{
  ...
  for ( std::vector<jobEntryType*>::iterator job = jobs_.begin(); job != jobs_.end(); ++job )
  {
    ...
    if ( !graph ) throw cms::Exception("TGraphWriter") << " Failed to load TGraph = " << (*job)->graphName_.data() << " from file = " << (*job)->inputFileName_ << " !!\n";

    edm::Service<cond::service::PoolDBOutputService> dbService;
    if ( !dbService.isAvailable() ) throw cms::Exception("TGraphWriter") << " Failed to access PoolDBOutputService !!\n";
